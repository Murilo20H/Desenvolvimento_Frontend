...
app.get("/listar-pagamentos-diarios", async (req, res) => {
    try {
        let dataReferencia = req.query.dataPagamento
        let pagamentoTotalDoDia = 0
        // filtro por data
        let pagamentos = await Pagamento.find({ dataPagamento: dataReferencia })
        for (let pagamento of pagamentos) {
            pagamentoTotalDoDia += pagamento.valorPagamento
        }
        return res.status(200).json({ pagamentos, pagamentoTotalDoDia })
    } catch (error) {
        return res.status(500).json(error)
    }
})
